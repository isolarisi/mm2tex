...
Функция ПолучитьТекстПараграфа(СтруктураЗаметки)
	ТекстПараграфа = СтруктураЗаметки;
	Возврат ТекстПараграфа;
КонецФункции

&НаКлиенте
Процедура ВыполнитьПреобразование(Команда) Экспорт
	ВыполнитьПреобразованиеНаКлиенте();
КонецПроцедуры

&НаКлиенте
Функция ВыполнитьПреобразованиеНаКлиенте() Экспорт
...
#Область РазборФайла
&НаСервере
Функция ВыполнитьПредварительноеПреобразование(ТекстДляПреобразования)
	Результат = ТекстДляПреобразования;
	//общие замены, которые не обрабатываются XML
	Результат =  СтрЗаменить(Результат, "<<","""");
	Результат =  СтрЗаменить(Результат, ">>","""");	
	Пока СтрНайти(Результат,"<html>",НаправлениеПоиска.СНачала) > 0 Цикл 
		НачалоHTML = СтрНайти(Результат,"<html>",НаправлениеПоиска.СНачала);
		КонецHTML = СтрНайти(Результат,"</html>",НаправлениеПоиска.СНачала);
		СтрокаДоHTML = Лев(Результат, НачалоHTML-1);
		СтрокаПослеHTML = Прав(Результат, СтрДлина(Результат) - (КонецHTML+6));
		СтрокаHTML =  СтрЗаменить(Результат, СтрокаДоHTML,"");
		СтрокаHTML =  СтрЗаменить(СтрокаHTML, СтрокаПослеHTML,"");
		//преобразуем тэги
		СтрокаHTML =  СтрЗаменить(СтрокаHTML, Символ(10)," ");
		СтрокаHTML =  СтрЗаменить(СтрокаHTML, "<b>","\textbf{");
		СтрокаHTML =  СтрЗаменить(СтрокаHTML, "</b>","}");
		СтрокаHTML =  СтрЗаменить(СтрокаHTML, "<strong>","\textbf{");
		СтрокаHTML =  СтрЗаменить(СтрокаHTML, "</strong>","}");
		
		HTMLДокумент =  Новый ФорматированныйДокумент;
		HTMLДокумент.УстановитьHTML(СтрокаHTML, Новый Структура());
		Текст = HTMLДокумент.ПолучитьТекст();
		Результат = СтрокаДоHTML+Текст+СтрокаПослеHTML;
	КонецЦикла;
	//общие замены, которые не обрабатываются XML
	Результат =  СтрЗаменить(Результат, "<<","""");
	Результат =  СтрЗаменить(Результат, ">>","""");	
	Результат =  СтрЗаменить(Результат, " TYPE=""NOTE""","");	
...
Функция РазобратьXML()
	...
	Документ.Прочитать(Объект.ИмяФайла);
	ТекстДокумента = Документ.ПолучитьТекст();
	ТекстДокумента = ВыполнитьПредварительноеПреобразование(ТекстДокумента);
	Документ.УстановитьТекст(ТекстДокумента);
